...
import type {
	Pattern,
	PatternDescription,
} from "$lib/interfaces/pattern-interface";
...
export const patternAPI = {
	...
	async loadPatterns() {
		try {
			...
			const descriptionsResponse = await fetch(
				"/data/pattern_descriptions.json",
			);
			const descriptionsData = await descriptionsResponse.json();
			const descriptions =
				descriptionsData.patterns as PatternDescription[];
			...
			const patternsPromises = data.map(async (pattern: string) => {
				try {
					...
					console.log(
						`Pattern ${pattern} content length:`,
						patternData.Pattern?.length || 0,
					);

					// Find matching description from JSON
					const desc = descriptions.find(
						(d) => d.patternName === pattern,
					);
					if (!desc) {
						console.warn(
							`No description found for pattern: ${pattern}`,
						);
					}
	...
	selectPattern(patternName: string) {
		...
		if (selectedPattern) {
			console.log(
				"Found pattern content (length: " +
					selectedPattern.Pattern.length +
					"):",
			...
			console.log(
				"First 100 chars:",
				selectedPattern.Pattern.substring(0, 100),
			);
			console.log(
				"Last 100 chars:",
				selectedPattern.Pattern.substring(
					selectedPattern.Pattern.length - 100,
				),
			);
			console.log(
				`Setting system prompt with content length: ${selectedPattern.Pattern.length}`,
			);
			console.log(
				`Content preview:`,
				selectedPattern.Pattern.substring(0, 100),
		...
		console.log(
			"System prompt store value after setting:",
			get(systemPrompt),
		);
	},
};
